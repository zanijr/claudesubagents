...
This gate is the **commit gate**, not the primary debugging loop. Dispatch prompts must
already include the subtask's exact verification command(s) and require the agent to iterate
edit→evaluate→diagnose internally until they pass before returning (see Phase 3 in SKILL.md).
An agent that returns work failing its own stated success criteria skipped its inner loop —
note that in the reroute feedback. The gate exists to catch what self-verification can't:
review-level issues (security, quality, conventions) and criteria the agent gamed or
misunderstood.
...
## Attempt History
{one line per prior attempt on this subtask: what it changed, which success criteria
passed/failed, review verdict — so this attempt builds on the lineage instead of
restarting blind}

## Verification
Run: {exact success-criteria command(s)}
Iterate edit→evaluate→diagnose until these pass BEFORE returning.

## Instructions
1. Fix ALL critical issues identified by the reviewer.
2. Do NOT introduce new features or refactor unrelated code.
3. Run the verification command(s) after fixing to ensure nothing broke.
...
3. If still failing, loop back to Step 4 — but if two consecutive reroutes leave the same
   criterion failing, that is a **plateau**: switch to the plateau intervention in
   `references/self-healing.md` instead of another same-direction reroute
